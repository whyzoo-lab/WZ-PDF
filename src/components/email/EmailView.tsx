  ...
  return (
    <li className="flex items-center gap-3 py-2 border-t border-gray-200 first:border-t-0">
      ...
      {canOpenHere && (
        <button
          onClick={() => onOpen(file)}
          className="shrink-0 px-2.5 py-1 text-xs rounded-full text-blue-700 hover:bg-blue-50 transition-colors"
        >
          {t('email.openHere')}
        </button>
      )}
      <button
        ...
        className="shrink-0 px-2.5 py-1 text-xs rounded-full bg-gray-100 text-gray-700 hover:bg-gray-200 transition-colors"
      >
        {t('email.download')}
      </button>
    </li>
 ...
 */
export function EmailView({ email, onOpenAttachment }: EmailViewProps) {
  ...
  return (
    <div className="h-full overflow-auto bg-gray-300 py-6 px-4">
      <article className="mx-auto max-w-3xl bg-white shadow-xl rounded-sm">
        {/* Envelope */}
        ...
        {body.blockedImages > 0 && (
          <div className="flex items-center gap-3 px-6 py-2.5 bg-amber-50 border-b border-amber-200 text-sm">
            ...
            <button
              ...
              className="shrink-0 px-3 py-1 rounded-full bg-amber-500/15 text-amber-900 hover:bg-amber-500/25 text-xs font-medium transition-colors"
            >
              {t('email.loadImages')}
            </button>
          </div>
        ...
        {hasBody ? (
          <div
            className="wz-email-body px-6 py-5 text-sm text-gray-900 break-words"
            dangerouslySetInnerHTML={{ __html: body.html }}
          />
        ) : (
          <p className="px-6 py-8 text-sm text-gray-500 text-center">{t('email.noBody')}</p>
        )}
