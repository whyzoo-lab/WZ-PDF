...
## 10. 풀스크린 수평 스와이프 네비게이션

**파일**: `src/components/viewer/FullscreenView.tsx` 만 수정

### 동작
| 제스처 | 결과 |
|---|---|
| 터치패드 두 손가락 → 오른쪽으로 쓸기 | 다음 페이지 |
| 터치패드 두 손가락 → 왼쪽으로 쓸기 | 이전 페이지 |

### 구현
기존 `onWheel` 핸들러를 확장. `deltaX` 누산기 ref(`deltaXAccRef`) 추가:

1. `|deltaX| > |deltaY|` 인 경우 → 수평 스와이프로 판단
2. `deltaX`를 누산 (`+=`)
3. 누산값이 **+80px** 초과 → 다음 페이지, **-80px** 미만 → 이전 페이지
4. 페이지 이동 후 누산값 리셋 (0)
5. 수직 스크롤 이벤트 발생 시 누산값 리셋 (대각선 스와이프 오작동 방지)

기존 수직 스크롤(`deltaY`) 쿨다운 로직과 Ctrl+wheel 줌 로직은 그대로 유지.
